...
The deck starts with one page. Navigate forward from the final page with the next control, a forward key, or a leftward swipe to create another blank page; the accumulated pages are restored in this browser on reload. The options panel can delete the current page (after confirmation) whenever more than one page exists; surviving pages retain their stable IDs and annotations.
