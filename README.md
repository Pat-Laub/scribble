...
The drawing tools are open as soon as the deck starts. The compact rail keeps the pen, highlighter, eraser, lasso selector, undo/redo, and all five colours in immediate reach; its ••• button opens width, pressure, ruled guides, clear, annotated PDF, and file transfer. Circle annotations with the lasso, then drag inside its selection box to move them or drag a corner handle to resize them proportionally. A contextual strip can copy, delete, or continue with the selection: Continue advances one slide, pastes the working at the top margin, and leaves it selected. Copied ink can also be pasted normally on any slide and is selected immediately for positioning. `Cmd/Ctrl+Enter`, `Cmd/Ctrl+C`, `Cmd/Ctrl+V`, and Delete/Backspace provide the same selection actions from a keyboard. Apple Pencil pressure sensitivity is on by default and can be toggled in the options. Width changes apply to future strokes; finished strokes keep the width they were drawn with. Ruled guides can be toggled and spaced closer together or farther apart; both settings stay local to the presenter and appear with each slide's ink in Overview and in PDF output. `Cmd/Ctrl+Z` undoes, `v` hides or restores ink, and `l` toggles ruled writing guides. Scribble over a stroke, or hold the right mouse/stylus barrel button, to erase it. A mouse wheel cycles the visible colours, and its middle button alternates the pen and highlighter. On iPad, fingers navigate from the first touch while Pencil input draws; elsewhere, mouse and touch input can doodle. Multi-contact gestures are ignored so a resting palm cannot accidentally navigate.
...
The writing canvas is 16:9 so its visible left and right edges agree with Overview and PDF pages on modern widescreen displays. Ink saved or exported by the earlier 16:10 version is automatically recentred once when it is first opened or imported.
